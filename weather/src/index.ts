...
const mapConditionToIcon = (type: string): string => {
...
async function getGoogleForecast(lat: number, lng: number, days: number) {
    ...
    if (data.error || !data.forecastDays) return null;
...
async function getOpenMeteoForecast(lat: number, lng: number, days: number) {
...
async function getGoogleCurrent(lat: number, lng: number) {
    if (!GOOGLE_WEATHER_API_KEY) return null;
...
async function getOpenMeteoCurrent(lat: number, lng: number) {
