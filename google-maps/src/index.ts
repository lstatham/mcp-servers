...
server.tool(
    "search_place",
    { query: z.string().describe("Place name or query to search for (e.g., 'Shinjuku Gyoen')") },
    async ({ query }) => {
        try {
...
server.tool(
    "get_place_details",
    { placeId: z.string().describe("Google Place ID (e.g., 'ChIJ51cu8IcbXWARiRtXIothAS4')") },
    async ({ placeId }) => {
        try {
...
server.tool(
    "geocode",
    { address: z.string().describe("Address to geocode (e.g., '1600 Amphitheatre Parkway, Mountain View, CA')") },
    async ({ address }) => {
        try {
